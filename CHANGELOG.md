...
- Optuna-Dashboard LLM support
- LLM config UI
- Update macOS Tunny UI to improve usability and support LLM config
- Function limitation notification when using community mode
- Run internet connection check when optunahub sampler is selected to avoid optunahub sampler runtime error
- Support indefinite sampling
  - If you set Number of Trials "negative int" or "inf", continue optimization indefinitely
- Notification when in-memory optimization is saved to a persistent file
- Warning message for ongoing Python installation
- User notifications for missing config file and loading failures

### Changed

- Change Config file format Json to Toml
- Storage settings handling
  - Since support LLM config
- RunDashboard command in Grasshopper Tool strip and Rhino command show storage config UI
- Benchmark component variables digits
- Clean window hierarchy
  - Tunny window become not always top
- GP sampler change to recommend to use Human-in-the-Loop
- Move MessageBox to Tunny.Core csproj
- Update Tunny UI storage config to new one

### Fixed

- Add null check for incomplete trials in OutputTrialItem
...
- Do not load existing configuration file.

## [v1.2.0] -2025-09-26

### Added

- GP:Optuna support constrained multi-objective optimization
- Python3 component compatibility check
  - If Python3 component is located in Grasshopper document, Tunny may crash Rhino.

### Changed

- Clean GrasshopperIO code
- Improve AUTO sampler to enhance multi-objective optimization
- Update NSGAII sampler to improve optimization speed
- Replace Chromium to Webview due to reduce file size
- Improve IsFail Check to use ordinal ignore comparison
- UI initialize error message more helpful
- Improve message box useability
- Make settings.json load error handling more user friendly

### Fixed

- Version update notification typo
- Benchmark component GenePool digit handling
- FishEgg deserialize error
- Check Git-scm exist to avoid optunahub sampler download error
- Objective value is always 3.0 and trial state is not FAIL if IsFail attribute is true
